            ...
            >
              <header className="p-5 md:p-6 border-b border-zinc-100 flex items-center justify-between bg-white sticky top-0 z-10">
                ...
                <div className="flex gap-2">
                  {role === 'staff' && (
                    <>
                      <label className="bg-blue-500 hover:bg-blue-600 text-white px-2 md:px-3 py-2 rounded-xl flex items-center gap-1.5 text-sm font-medium cursor-pointer">
                        {isProcessing ? <Loader2 className="w-4 h-4 animate-spin" /> : <Upload className="w-4 h-4" />}
                        <span className="hidden sm:inline">AI Upload</span>
                        <input type="file" accept="image/*" capture="environment" className="hidden" onChange={handleAIUpload} disabled={isProcessing} />
                      </label>
                ...
                {cabinets.length === 0 ? (
                  ...
                  <div key={cab}>
                    <div className="sticky top-0 bg-zinc-100/90 backdrop-blur-sm px-4 md:px-5 py-1.5 border-y border-zinc-200 flex items-center gap-2">
                      ...
                      <span className="text-xs font-bold text-zinc-500 uppercase tracking-widest">Cabinet {cab}</span>
                    </div>
