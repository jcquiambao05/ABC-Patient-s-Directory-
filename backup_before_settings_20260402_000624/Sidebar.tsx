...
export default function Sidebar({ activeTab, setActiveTab, onLogout, role }: Props) {
  return (
    <>
      ...
      <div className="hidden md:flex md:w-16 lg:w-72 bg-zinc-950 text-zinc-400 h-screen flex-col border-r border-zinc-800 flex-shrink-0">
        {/* Logo */}
        <div className="p-5 lg:p-7 flex items-center gap-3 text-white overflow-hidden">
          ...
          <span className="font-bold text-xl tracking-tight hidden lg:block truncate">ABCare OmniFlow</span>
        </div>
        ...
        <nav className="flex-1 px-2 lg:px-4 space-y-1 mt-2">
          {nav.map(item => (
            <button key={item.id} onClick={() => setActiveTab(item.id)}
              title={item.label}
              className={`w-full flex items-center gap-3 px-3 lg:px-4 py-3.5 rounded-xl transition-all ${
        ...
        <div className="p-2 lg:p-4 border-t border-zinc-800 space-y-2 lg:space-y-3">
          {/* Role badge — full on desktop, dot only on tablet */}
          <div className="hidden lg:block bg-zinc-900/50 rounded-xl p-3">
          ...
          {/* Tablet: just the role dot */}
          <div className={`flex md:flex lg:hidden justify-center py-1`}>
            <div className={`w-2.5 h-2.5 rounded-full ${role === 'admin' ? 'bg-blue-400' : 'bg-emerald-400'}`} title={role === 'admin' ? 'Doctor / Admin' : 'Staff'} />
          </div>
          <button onClick={onLogout} title="Sign Out"
            className="w-full flex items-center justify-center lg:justify-start gap-3 px-3 lg:px-4 py-3.5 rounded-xl hover:bg-zinc-900 hover:text-red-400 text-zinc-400 transition-all">
            <LogOut className="w-[22px] h-[22px] flex-shrink-0" />
            <span className="font-medium text-lg hidden lg:block">Sign Out</span>
          </button>
        </div>
      </div>
